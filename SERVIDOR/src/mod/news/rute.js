...
router.get("/lastone", getLastOne);
router.get("/lasthree", getLastThree);
router.get("/:id", one);
router.post("/", add);
router.put("/", remove);
...
async function getLastThree(req, res) {
  try {
    const items = await controller.getLastThree();
    response.success(req, res, items, 200);
  } catch (error) {
    response.error(req, res, error.message, 500);
  }
...
async function getLastOne(req, res) {
  try {
    const items = await controller.getLastOne();
    response.success(req, res, items, 200);
  } catch (error) {
    response.error(req, res, error.message, 500);
  }
